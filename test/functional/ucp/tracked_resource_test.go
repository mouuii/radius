...
func Test_TrackedResources(t *testing.T) {
	...
	rgc, err := v20220901privatepreview.NewResourceGroupsClient(&aztoken.AnonymousCredential{}, sdk.NewClientOptions(options.Connection))
	require.NoError(t, err)
	rc, err := v20220901privatepreview.NewResourcesClient(&aztoken.AnonymousCredential{}, sdk.NewClientOptions(options.Connection))
	require.NoError(t, err)
	ac, err := v20220315privatepreview.NewApplicationsClient(resourceGroupID.String(), &aztoken.AnonymousCredential{}, sdk.NewClientOptions(options.Connection))
	require.NoError(t, err)
	exc, err := v20220315privatepreview.NewExtendersClient(resourceGroupID.String(), &aztoken.AnonymousCredential{}, sdk.NewClientOptions(options.Connection))
	require.NoError(t, err)

	rg, err := rgc.CreateOrUpdate(ctx, "radius", "local", resourceGroupID.Name(), v20220901privatepreview.ResourceGroupResource{Location: to.Ptr(v1.LocationGlobal)}, nil)
	require.NoError(t, err)
	log("Created resource group", rg)

	t.Run("Resource group starts empty", func(t *testing.T) {
		resources := []*v20220901privatepreview.GenericResource{}
	...
	t.Run("Create resources", func(t *testing.T) {
		for i := 0; i < 3; i++ {
			a, err := ac.CreateOrUpdate(ctx, fmt.Sprintf("app-%d", i), v20220315privatepreview.ApplicationResource{
			...
			poller, err := exc.BeginCreateOrUpdate(ctx, fmt.Sprintf("ex-%d", i), v20220315privatepreview.ExtenderResource{
				Location: to.Ptr(v1.LocationGlobal),
				Properties: &v20220315privatepreview.ExtenderProperties{
					Environment:          to.Ptr(options.Workspace.Environment),
	...
	t.Run("Resource group contains resources", func(t *testing.T) {
		expected := []*v20220901privatepreview.GenericResource{}

		for i := 0; i < 3; i++ {
			expected = append(expected, &v20220901privatepreview.GenericResource{
