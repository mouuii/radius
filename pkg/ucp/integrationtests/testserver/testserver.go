...
func StartWithETCD(t *testing.T, configureModules func(options modules.Options) []modules.Initializer) *TestServer {
	...
	queueProvider := queueprovider.New(backend.UCPProviderName, queueOptions)

	operationStorageClient, err := dataProvider.GetStorageClient(ctx, backend.UCPProviderName+"/operationStatuses")
	require.NoError(t, err)

	queueClient, err := queueProvider.GetClient(ctx)
	require.NoError(t, err)

	statusManager := statusmanager.New(operationStorageClient, queueClient, backend.UCPProviderName, "global")
