...
func (s *Service) Initialize(ctx context.Context) (*http.Server, error) {
	...
	storageClient, err := s.storageProvider.GetStorageClient(ctx, "System.Resources/operationstatuses")
	if err != nil {
		return nil, err
	}

	statusManager := statusmanager.New(storageClient, queueClient, "System.Resources", s.options.Location)
