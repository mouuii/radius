...
func (p *ProxyController) EnqueueTrackedResourceUpdate(ctx context.Context, id resources.ID, apiVersion string) error {
	...
	// Create a serviceCtx to refer to the tracked resource.
	...
	// Create the database entry for the tracked resource. It's OK for us to do this without a lock, because
	// the background work-item will synchronize our state with the resource provider's state.
	//
	...
	err := p.StorageClient().Save(ctx, &store.Object{Metadata: store.Metadata{ID: trackingID.String()}, Data: entry})
	if err != nil {
		return err
	}
