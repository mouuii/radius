...
//
// If the downstream resource is still in progress (provisioning state is non-terminal) then this function will return
// an InProgressErr immediately.
...
func (u *Updater) run(ctx context.Context, id resources.ID, trackingID resources.ID, destination *url.URL, apiVersion string) error {
	...
	if data == nil {
		...
		return u.Store.Save(ctx, &obj, store.WithETag(etag))
	}
}
